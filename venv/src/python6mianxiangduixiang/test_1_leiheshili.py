...
# 通常，如果没有合适的继承类，就使用object类，这是所有类最终都会继承的类。    类似于java

# 定义好了Student类，就可以根据Student类创建出Student的实例，创建实例是通过类名+()实现的：
bart = Student()  # 表示创建的对象
...
class Student(object):
    ...
    def __init__(self, name='luelue', score=56):
        self.name = name
        self.score = score
...
# bart = Student(score=44)  # 表示创建的对象
# print(bart.name, '-->', bart.score)
# bart = Student(name='就是这个')  # 表示创建的对象
# print(bart.name, '-->', bart.score)
# bart = Student('is this')  # 表示创建的对象
# print(bart.name, '-->', bart.score)
...
bart.age = 8
print(bart.age)
print(lisa.age)  # 因为没有给lisa.age绑定数据,所以lisa.age是没有值得
